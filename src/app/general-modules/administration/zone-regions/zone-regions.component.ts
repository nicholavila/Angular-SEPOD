...
@Component({
    selector: 'app-zone-regions',
    templateUrl: './zone-regions.component.html',
    styleUrls: ['./zone-regions.component.css']
})
export class ZoneRegionsComponent implements OnInit {
    dataStatus = 'fetching'
    dataList = []
    dataForm: FormGroup
    selectedIndex = -1
    ...
    breadCrum = [
        {
            link: '/user/zone',
            value: 'Zones',
            params: { id: this.zoneId, name: this.zoneName }
        }
    ]
    countriesList: any
    statesList: any
    ...
    constructor(
        ...
        this.zoneId = this.route.snapshot.queryParamMap.get('id')
        this.zoneName = this.route.snapshot.queryParamMap.get('name')

        this.breadCrum.push({
            link: '/user/zone-regions',
            params: { id: this.zoneId, name: this.zoneName },
            value: this.zoneName
        })
        ...
        this.dataForm = this.fb.group({
            ...
            region_type: new FormControl('state', [Validators.required]),
            region_ids: new FormControl(null, [Validators.required])
        })
        ...
        this.ds.stateList().subscribe((resp: any) => {
            if (resp.success === false) {
                this.alert.error(resp.errors.general)
    ...
    setPagination(page: number) {
        ...
        this.router.navigate(['/user/zone-regions'], { queryParams: filtersParam, replaceUrl: true })
    }

    openModal(formModal, index) {
        this.modalTitle = 'Add New Zone Region'
        if (index > -1) {
            this.selectedIndex = index
            this.dataForm.controls.id.setValue(this.dataList[index].id)
            this.dataForm.patchValue(this.dataList[index])
            this.modalTitle = 'Edit Zone Region'
        }
    ...
    save(f: any) {
        ...
        const params = {
            ...
            zone_id: +this.zoneId,
            region_type: this.dataForm.value.region_type,
            region_ids: this.dataForm.value.region_ids
        }
    ...
    changeRegionType() {
        this.dataForm.controls.region_ids.setValue([])
    }
