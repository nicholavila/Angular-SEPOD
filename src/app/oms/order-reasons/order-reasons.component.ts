...
@Component({
    selector: 'app-order-reasons',
    templateUrl: './order-reasons.component.html',
    styleUrls: ['./order-reasons.component.css']
})
export class OrderReasonsComponent implements OnInit, OnDestroy {
    dataStatus = 'fetching'
    dataList = []
    dataForm: FormGroup
    selectedIndex = -1
    ...
    setPagination(page: number) {
        ...
        this.router.navigate(['/user/order-reason'], { queryParams: filtersParam, replaceUrl: true })
    }

    openModal(formModal, index) {
        this.modalTitle = 'Add New Order Reason'
        if (index > -1) {
            this.selectedIndex = index
            this.dataForm.controls.id.setValue(this.dataList[index].id)
            this.dataForm.patchValue(this.dataList[index])
            this.modalTitle = 'Edit Order Reason'
        }
