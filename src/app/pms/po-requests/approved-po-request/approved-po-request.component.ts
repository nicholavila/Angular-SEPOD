...
export class ApprovedPoRequestComponent implements OnInit, OnDestroy {
    ...
    showItem(i: any, id) {
        ...
        if (this.dataList[i].approvedPoItems) {
            return
        }
        this.dataList[i].dataStatusInner = 'fetching'
        this.ds.approvedPoItems({ id }).subscribe((resp: any) => {
            if (resp.success === true) {
                this.dataList[i].approvedPoItems = resp.data
                this.dataList[i].dataStatusInner = 'done'

                this.dataList[i].totalCost = 0
                this.dataList[i].approvedPoItems.forEach(e => {
    ...
    sumLastCP(data: any) {
        ...
        // console.log('Sum Last CP', sum)
        // console.log('Last CP', data)
        return sum
    }
