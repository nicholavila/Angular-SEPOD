...
export class ProductListComponent implements OnInit, OnDestroy {
    ...
    downloadZip(product_id) {
        this.Loading = true
        this.ds.downloadZip({ product_id }).subscribe((resp: any) => {
    ...
    getList() {
        ...
        list.subscribe((resp: any) => {
            if (resp.success === false) {
                ...
                this.router.navigate([this.api.checkUser() + '/products-list'], { queryParams: params, replaceUrl: true })
    ...
    setPagination(page: number) {
        ...
        this.router.navigate([this.api.checkUser() + '/products-list'], { queryParams: filtersParam, replaceUrl: true })
    }
    ...
    openModal(variantModal, mainId, index, innerId, innerIndex) {
        ...
        if (index > -1) {
            this.selectedIndex = index
            this.productVariantSKUSplit = this.dataList[this.selectedIndex].variants[this.selectedInnerIndex].base_product_variants.sku.split('-', 3)
            this.productSKU = this.productVariantSKUSplit[0] + '-' + this.productVariantSKUSplit[1]

            this.dataForm.patchValue(this.dataList[this.selectedIndex].variants[this.selectedInnerIndex].base_product_variants)
            this.dataForm.controls.id.setValue(this.dataList[this.selectedIndex].variants[this.selectedInnerIndex].base_product_variants.id)
