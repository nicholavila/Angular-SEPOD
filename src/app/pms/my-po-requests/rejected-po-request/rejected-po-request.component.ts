...
export class RejectedPoRequestComponent implements OnInit, OnDestroy {
    ...
    showItem(i: any, id) {
        ...
        if (this.dataList[i].rejectedItems) {
            return
        }
        this.dataList[i].dataStatusInner = 'fetching'
        this.ds.rejectedPoRequestItems({ id }).subscribe((resp: any) => {
            if (resp.success === true) {
                this.dataList[i].rejectedItems = resp.data
                this.dataList[i].dataStatusInner = 'done'

                this.dataList[i].totalCost = 0
                this.dataList[i].rejectedItems.forEach(e => {
